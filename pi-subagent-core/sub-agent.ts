/**
 * sub-agent.ts — the optional pi extension entry for the sub-agent UI.
 *
 * Registers:
 *   - the above-editor agent widget (ui/agent-widget.ts) — live per-agent
 *     stats with spinner, activity lines, and token annotations;
 *   - the below-editor FleetView (ui/fleet-list.ts) — navigable main+agents
 *     list with a conversation-viewer overlay;
 *   - the `/agents` command — list the session's agents, Enter opens the
 *     conversation viewer.
 *
 * Configuration is file-based only (settings.ts), read from
 ...
 *
 * The dispatch core (index.ts) stays a plain library: this extension only
 * READS the process-global `monitor` that spawnAgent notifies (pi loads every
 * extension with moduleCache:false, so the monitor lives on globalThis under
 * a registered symbol — see README "Wiring").
 *
...
import { loadCoreSettings, type SubagentCoreSettings, type WidgetMode } from "./settings.ts";
import { AgentWidget } from "./ui/agent-widget.ts";
import { FleetList, type FleetUICtx } from "./ui/fleet-list.ts";
import type { WidgetUICtx } from "./ui/agent-widget.ts";
import { setModelCatalog } from "./ui/shared.ts";
...
const UI_KEY = Symbol.for("@fyeeme/pi-subagent-core/ui");
...
export default function subagentCoreUiExtension(pi: ExtensionAPI): void {
