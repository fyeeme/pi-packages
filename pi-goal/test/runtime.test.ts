/**
 * pi-goal — runtime tests.
 *
 * Scenarios ported from oh-my-pi `test/goals/goal-runtime.test.ts`
 * (bun:test → vitest; the harness shape is carried over). Together they pin
 * the budget-accounting semantics that must never drift: cache writes count,
 * cache reads do not; wall-clock accounting advances in whole persisted
 * seconds; budget-limit steering fires once per goal id; interrupts pause;
 * resume auto-pauses.
 */
...
import type { Goal, GoalModeState, GoalRuntimeEvent, GoalTokenUsage } from "../src/state.ts";
import { escapeXmlText } from "../src/template.ts";
...
describe("goal runtime", () => {
	...
	it("completeGoalFromTool clears enabled and flips status to complete with mode exiting", async () => {
		const harness = createHarness({
			state: {
				enabled: true,
				mode: "active",
	...
	it("recordImpossibleReport bumps the dispute counter and persists", async () => {
		const harness = createHarness({ state: { enabled: true, mode: "active", goal: createGoal() } });

		const first = await harness.runtime.recordImpossibleReport();
		expect(first.impossibleReports).toBe(1);
		const second = await harness.runtime.recordImpossibleReport();
		expect(second.impossibleReports).toBe(2);
		expect(harness.getState()?.goal.impossibleReports).toBe(2);
	});

	it("recordImpossibleReport requires an accounting-active goal", async () => {
		const harness = createHarness({
			state: { enabled: false, mode: "active", goal: createGoal({ status: "paused" }) },
		});
		await expect(harness.runtime.recordImpossibleReport()).rejects.toThrow(
			"cannot report impossibility because no active goal exists",
		);
	});

	it("pauseGoal carries an explicit reason (impossible outcomes)", async () => {
		const harness = createHarness({ state: { enabled: true, mode: "active", goal: createGoal() } });

		const paused = await harness.runtime.pauseGoal({ reason: "impossible-confirmed" });

		expect(paused?.enabled).toBe(false);
		expect(paused?.goal.status).toBe("paused");
		expect(paused?.reason).toBe("impossible-confirmed");
		expect(harness.getState()?.reason).toBe("impossible-confirmed");
	});
	...
	it("goal ids are unique (randomUUID replaced omp Snowflake)", async () => {
		const harness = createHarness();
		const first = await harness.runtime.createGoal({ objective: "One" });
		harness.setState(undefined);
