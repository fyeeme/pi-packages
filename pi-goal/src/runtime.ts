/**
 * pi-goal — goal runtime.
 *
 * Ported from oh-my-pi `packages/coding-agent/src/goals/runtime.ts`
 * behavior-for-behavior: budget accounting (token delta since turn baseline +
 * wall-clock seconds), budget-limited flips with one-shot steering, pause on
 * interrupt, resume/pause/drop/complete transitions, and the serialized
 * accounting tail. The host interface is implementation-agnostic and maps to
 * pi's extension API in index.ts:
 *
 *   omp host surface                          → pi adaptation
 *   ─────────────────────────────────────────────────────────────────
 *   getState/setState (session field)         → closure state
 *   getCurrentUsage (session stats tokens)    → entries scan (same sums as
 *                                               pi's getSessionStats)
 *   emit(goal_updated session event)          → pi.events.emit("goal_updated")
 *   persist (appendModeChange goal/goal_paused/none)
 *                                             → pi.appendEntry custom entries
 *   sendHiddenMessage                         → pi.sendMessage display:false
 *
 * Prompts load from static .md assets next to this module (published with the
 * package). Loaded at runtime: pi loads extensions through jiti, which does
 * not support bundler-style text imports (pi-todo/pi-dynamic-workflows
 * precedent). omp `Snowflake.next()` ids become `crypto.randomUUID()`.
 */

import { readFileSync } from "node:fs";
import * as path from "node:path";
import { fileURLToPath } from "node:url";
import type { Goal, GoalBudgetSteering, GoalModeState, GoalRuntimeEvent, GoalTokenUsage } from "./state.ts";
import { cloneGoal, cloneState } from "./state.ts";
...
const goalBudgetLimitPrompt = readFileSync(path.join(promptsDir, "goal-budget-limit.md"), "utf8");
const goalContinuationPrompt = readFileSync(path.join(promptsDir, "goal-continuation.md"), "utf8");
const goalModeActivePrompt = readFileSync(path.join(promptsDir, "goal-mode-active.md"), "utf8");
...
export class GoalRuntime {
	...
	async pauseGoal(options?: { reason?: GoalModeState["reason"] }): Promise<GoalModeState | undefined> {
		return await this.#withAccounting(async () => {
			await this.#flushUsageLocked("suppressed");
			const state = this.#getStateClone();
			if (!state?.goal) return undefined;
	...
	/**
	 * Record one agent-reported impossibility (CC 2.1.261's `impossible`
	 * channel): flush usage, bump the dispute counter, persist. The caller
	 * (goal tool) then runs the independent evaluator — confirmation pauses
	 * the goal, refutation leaves it active and the count feeds the cap.
	 */
	async recordImpossibleReport(): Promise<Goal> {
		return await this.#withAccounting(async () => {
			await this.#flushUsageLocked("suppressed");
			const state = this.#getStateClone();
			if (!state?.enabled || !isAccountingStatus(state.goal)) {
			...
			state.goal.impossibleReports = (state.goal.impossibleReports ?? 0) + 1;
			state.goal.updatedAt = this.#now();
			await this.#commitState(state, { persist: "goal" });
			return state.goal;
		});
