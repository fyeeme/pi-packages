...
export function assertPatchableAssistantMessageComponent(value: unknown): { prototype: AssistantMessageComponentPrototype } {
	if (!value || (typeof value !== "function" && typeof value !== "object")) {
		throw new Error("Thinking UI patch failed: AssistantMessageComponent export is missing or invalid.");
	}

	const prototype = (value as { prototype?: unknown }).prototype;
	if (!prototype || typeof prototype !== "object") {
		throw new Error("Thinking UI patch failed: AssistantMessageComponent.prototype is missing.");
	}
	...
	if (missingMethods.length > 0) {
		throw new Error(`Thinking UI patch failed: AssistantMessageComponent prototype is incompatible (missing ${missingMethods.join(", ")}).`);
	}

	return value as { prototype: AssistantMessageComponentPrototype };
}

export function assertThinkingUITheme(value: unknown): ThinkingThemeLike {
	if (!value || typeof value !== "object") {
		throw new Error("Thinking UI patch failed: interactive theme export is missing or invalid.");
	}

	try {
		const candidate = value as Record<string, unknown>;
		if (typeof candidate.fg !== "function" || typeof candidate.bold !== "function") {
			throw new Error("Thinking UI patch failed: interactive theme export is incompatible.");
		}
	} catch (error) {
...
function getPackageRoot(packageName: string): string {
	...
	try {
		...
		throw new Error(`Thinking UI patch failed: could not resolve ${packageName} package root. Pi internals may be unavailable or moved.`, {
			cause: error,
		});
	}
	...
	try {
		...
		throw new Error(`Thinking UI patch failed: could not derive ${packageName} package root from ${entryUrl}.`, {
			cause: error,
		});
	}
}
...
export async function importPiCodingAgentInternal<TModule>(relativePath: string): Promise<TModule> {
	...
	try {
		...
		throw new Error(`Thinking UI patch failed: could not import internal module "@earendil-works/pi-coding-agent/${relativePath}". Pi internals may have moved.`, {
			cause: error,
		});
	}
}
...
async function installPatch(): Promise<() => void> {
	...
	const theme = assertThinkingUITheme(rawTheme);
	...
	const reportFallback = (stage: string, error: unknown): void => {
		console.warn(`Thinking UI patch warning: falling back to Pi renderer during ${stage}.`, error);
	};

	const fallbackErrorMessage = "Thinking UI patch failed: Pi internals are incompatible and fallback rendering also failed.";
		...
		try {
			...
			for (const content of message.content) {
				...
				if (content.type === "thinking" && thinkingBlocks.length > 0 && !renderedThinking) {
					this.contentContainer.addChild(new ThinkingUIComponent(theme, message.timestamp, thinkingBlocks, resolveThinkingMessageScope(message)));
	...
	try {
		...
		try {
			...
			throw new Error("Thinking UI patch failed: AssistantMessageComponent prototype patching failed and rollback was incomplete.", {
				cause: { installError: error, rollbackError },
			});
		}

		throw new Error("Thinking UI patch failed: AssistantMessageComponent prototype is incompatible with thinking-ui patching.", { cause: error });
	}
...
export async function retainThinkingUIPatch(): Promise<() => Promise<void>> {
